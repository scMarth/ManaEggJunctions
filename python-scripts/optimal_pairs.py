...
The algorithm works by iterating from the lowest level eggs (1) to the higher level eggs.
Because we know the cost of the base case (level 1 eggs have cost 1), we can use them to calculate
the costs of eggs that use some combination of these eggs.

1.) The cost of an level X egg must be at least X

    Proof of 1.) 

Proof of Correctness (Induction):

If an egg X minimum cost junction pair is one that hasn't been explored, then one of those eggs
has a level greater to or equal to X, because by the time we get to X, we would have explored all eggs
with a lower level than X.

Assume that an egg X has a minimum cost junction pair A in which one egg in the pair hasn't been explored, and
has at least one other pair B in which both eggs are explored. Because at least one egg in the optimal pair
hasn't been explored, it contains an eg which has a level equal to or greater than the level of X.

Thus, by 1.), the cost of the 'minimum cost' junction pair A is at least X + (X-a). a could be anything...

Pair B, has a cost of at most (X-1) + (X-1) = 2X - 2. 
